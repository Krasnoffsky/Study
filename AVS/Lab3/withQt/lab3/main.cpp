...
system_clock::time_point timeEnd;
nanoseconds *timeWrite;
nanoseconds *timeRead;
...
/*
void BenchTimer(char type[], char task[], int number_try) //timer counting in nanosecs
{
    if (strcmp(task, "START") == 0){
        ...
        if (strcmp(type, "READ") == 0){
            timeRead[number_try] = timeEnd->tv_nsec - timeStart->tv_nsec;
        }
        else if (strcmp(type, "WRITE") == 0){
            timeWrite[number_try] = timeEnd->tv_nsec - timeStart->tv_nsec;
        }
    }
}
*/
void BenchTimer(string type, string task, int number_try)
{
    if (task == "START"){
        ...
        if (type == "WRITE"){
            timeWrite[number_try] = duration_cast<nanoseconds>(timeEnd - timeStart).count();
        }
        else if (type == "READ"){
            timeRead[number_try] = duration_cast<nanoseconds>(timeEnd - timeStart).count();
        }
    }
}
...
double AverageTimeCounter(string type, int size)
{
    ...
    if (type == "WRITE"){
        for (int i = 0; i < size; i++){
            average += duration_cast<seconds>(timeWrite[i]);
        }
        average /= size;
    }
    else if (type == "READ"){
        for (int i = 0; i < size; i++){
            average += duration_cast<seconds>(timeRead[i]);
        }
        average /= size;
    }
...
{
    absErrorWrite = abs((double)(timeWrite[number_try]) - timeCountWrite[number_try]);
    absErrorRead = abs(duration_cast<seconds>(timeRead[number_try]) - timeCountRead[number_try]);
    realErrorWrite = absErrorWrite / timeWrite[number_try] * 100;
    realErrorRead = absErrorRead / timeRead[number_try] * 100;
...
void FileWriter(int block, int num)
{
    ...
    output->writeTime = abs(duration_cast<seconds>(timeWrite[num - 1]));
    output->readTime = abs(duration_cast<seconds>(timeRead[num - 1]));
...
int main (int argc, char *argv[])
{
    ...
    if (memory_type == "HDD" || memory_type == "SSD"){
        ...
        timeWrite = new duration <long long int>[number_of_tests];
        timeRead = new duration <long long int> [number_of_tests];
        ...
        for (int i = 0; i < number_of_tests; i++){
            ...
            Accuracy(i);
            cout << i + 1 <<") Write = " << duration_cast<seconds>(timeWrite[i]) << " Read = " << duration_cast<seconds>(timeRead[i]) << endl;
